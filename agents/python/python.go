...
func NewDefaultPythonAgentLocal(ctx context.Context, model model.ToolCallingChatModel, localConfig *pyexecutor.LocalOperatorConfig) *PythonAgent {
	ddgTool, err := duckduckgo.NewDefaultDuckDuckGoTool(ctx)
	if err != nil {
		fmt.Printf("Error creating DuckDuckGo tool: %v", err)
		return nil
	}
	pythonTool, err := pyexecutor.NewPythonToolLocal(ctx, localConfig)
	if err != nil {
	...
	instruction :=
		`	
			...
			依赖管理：如果代码需要第三方库，尽量说明如何安装（例如“使用pip install安装”），或在允许的环境中自动安装（需确认权限）。
